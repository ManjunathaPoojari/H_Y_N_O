...
public interface YogaSessionRepository extends JpaRepository<YogaSession, Long> {
    ...
    List<YogaSession> findByTrainer_Id(Long trainerId);
    ...
    List<YogaSession> findUpcomingByTrainerId(@Param("trainerId") Long trainerId, @Param("date") LocalDate date);

    @Query("SELECT s FROM YogaSession s WHERE s.patient.id = :patientId AND s.trainer.id = :trainerId")
    List<YogaSession> findByPatientIdAndTrainerId(@Param("patientId") String patientId, @Param("trainerId") Long trainerId);
    ...
    List<YogaSession> findByTrainerIdAndStatus(@Param("trainerId") Long trainerId, @Param("status") YogaSession.SessionStatus status);
}
