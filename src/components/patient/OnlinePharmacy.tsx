...
export const OnlinePharmacy = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      if (user) {
        try {
          const fetchedOrders = await api.pharmacy.getOrders();
          const userOrders = fetchedOrders.filter((o: any) => o.patientId === user.id);
  ...
  const handleCheckout = async () => {
    ...
    const orderData = {
      ...
      patientPhone: (user as any).phone || 'N/A',
      items: cart.map(item => ({
      ...
      totalAmount: calculateTotal() + deliveryCharge,
      status: 'PENDING',
      deliveryAddress: (user as any).address || 'Default Address',
      paymentMethod: 'CASH_ON_DELIVERY',
      paymentStatus: 'PENDING'
    };

    try {
      const newOrder = await api.pharmacy.createOrder(orderData);
